	...
	if (fp = fopen(_filepath.c_str(), "rb")) {
		...
		while (true) {
			...
			for (int i = 0; i < windowSize && (currentBase + i) < totalFrames; i++) {
				//cout << "CURRENT: " << currentBase + i << endl;
	...
	while (true) {
		...
		if (outfds > 0) {
			...
			if (msgrecv.packet_type == 'r') {
				...
				//cout << " HDR: " << msgrecv.header << " WAIT SEQ: " << waitingSeq << endl;
		...
		else { // Send SENDFILE again if nothing is recieved
			//sendto(s, (const char*)&msgfrm, sizeof(msgfrm), 0, (struct sockaddr*) &sa_in, sizeof(sa_in));
		}
	}
...
void handleList(FILE *logfile) {
	...
	LPCWSTR _filepath = L"G:\\My Documents\\COMP445\\L3DEMO\\Server\\Server\\*";
