		...
		do {
			...
			if (strcmp(three_way_send.direction, "quit") != 0) {
				...
				if(direction.compare("rename") == 0) {
					// Ask for file from server to be renamed
			...
			while (true) {
				...
				//cout << "Waiting for server for final handshake..." << endl;
				FD_ZERO(&readfds);
				FD_SET(s, &readfds);  //always check the listener
				...
				//cout << "Server message received: " << mfr.header << endl;
				//if (mfr.header == 's') {
				break;
				//}
				}
...
void handleGet(FILE *logfile) {
	...
	while (fileExists(three_way_send.file_name)) {
		...
		if(strcmp(three_way_send.file_name,"n") == 0 || strcmp(three_way_send.file_name,"N") == 0) {
			...
			for(int i = 0; i < 4; i++) // Drop issues; resend this much
				sendto(s, (const char*)&msgfrm, sizeof(msgfrm), 0, (struct sockaddr*) &sa_in, sizeof(sa_in));
			return;
		}
	}

	// File name is good, start transfer
	...
	// Just listen for incoming packets
	while (true) {
		FD_ZERO(&readfds);
		FD_SET(s, &readfds);
		...
		if (outfds > 0) {
			...
			if (msgrecv.packet_type == 'r') {
				// Inform user to start sending file
				...
				// Only update sending frame if it's the right one
				//cout << " RECV: " << msgrecv.sequence << " WAIT SEQ: " << waitingSeq << endl;
				if (msgrecv.sequence == waitingSeq) {
					...
					//cout << "Sequence " << msgrecv.sequence << " rejected." << endl;
					// Unwanted packet
					memset(&msgfrm, 0, sizeof(msgfrm));
					strcpy(msgfrm.data, "NAK");
					msgfrm.packet_type = 'm';
					msgfrm.sequence = waitingSeq; // Same sequence
					totalPackets++;
					totalBytes += msgrecv.header;
				}

				// Send either ACK or NAK packets
				sendto(s, (const char*)&msgfrm, sizeof(msgfrm), 0, (struct sockaddr*) &sa_in, sizeof(sa_in));
			}
		} else {
			sendto(s, (const char*)&msgfrm, sizeof(msgfrm), 0, (struct sockaddr*) &sa_in, sizeof(sa_in));
...
void handlePut(FILE *logfile) {
	...
	while (true) {
		...
		if (outfds > 0) {
			...
			if(strcmp(msgrecv.data, "SENDFILE") == 0) { // Allowed to start sending file
				break;

			} else if(strcmp(msgrecv.data, "RENAMEFILE") == 0) { // Initialize file rename
			...
			// No response from receiver; packet must have been dropped = resend
			sendto(s, (const char*)&msgfrm, sizeof(msgfrm), 0, (struct sockaddr*) &sa_in, sizeof(sa_in));
		}
	}
	...
	if (fp = fopen(_filepath.c_str(), "rb")) {
		...
		// Get file length and total frames
		fseek(fp, 0, SEEK_END);
		...
		while(true) {
			...
			for (int i = 0; i < windowSize && (currentBase + i) < totalFrames; i++) {
				...
				msgfrm.sequence = currentBase + i; // gets larger
				msgfrm.packet_type = 'm';
			...
			for (int j = 0; j < windowSize; j++) {
				...
				if (outfds > 0) {
					...
					//cout << "MSG: " << msgrecv.data << " SEQ: " << msgrecv.sequence << endl;
					// Correct packet sent
					if (msgrecv.packet_type == 'm') {
						if (strcmp(msgrecv.data, "ACK") == 0) {
							// Comparison to currentBase allows to ignore delayed packets
							if (msgrecv.sequence > currentBase)
								currentBase = msgrecv.sequence; // Assign base for next packet
						...
						// Dropped or delayed packet
							...
							fseek(fp, currentBase * MAX_BUFFER_SIZE, SEEK_SET); // Recalculate fp (file pointer) position
							//cout << "CURNAK: " << currentBase << endl;
			...
			if (!isNak) currentBase++; // Only increment base to next sequence if it's not a NAK
			//cout << "CURRENT: " << currentBase << " LIMIT: " << totalFrames << endl;
